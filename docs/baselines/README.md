...
| `2026-09-06-postcutover-kmt-fly-dev.txt` | `kmt.fly.dev` at 23:13Z on 2026-09-06, after the cutover landed (`KMT_CANONICAL_HOST=kensmobiletire.com`, machine v171): every route 301s to the canonical host except `/api/health`. The closest before for a flip-only diff is `postheaders-precutover` (v104), but that capture aged 12h before the flip (67 machine versions) — the diff still proves the redirect is total, health-exempt and header-carrying, but also carries the day's other backend work (mail-seam secrets, `x-kmt-service-area`, a `HEAD /api/health` fix, a catalog refresh), enumerated in this file's own notes rather than isolated. Verification disposition: PASS. |
| `2026-09-06-postcutover-kensmobiletire-com.txt` | `kensmobiletire.com` at 23:14Z on 2026-09-06, the canonical host post-cutover: full landed surface (six security headers + HSTS, real `robots.txt`, `/api/nope` 404, `TRACE` 405, brand caching), catalog shape intact (6,169 tires / 511 sizes / 7 fields). Diffed against `precutover-kensmobiletire-com` (v87, the full-day before, 84 versions earlier); every changed line attributed to a specific PR or intervening deploy in this file's own notes. No finding. |
