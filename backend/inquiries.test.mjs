...
test('the growable workflow moves forward through new, replied and closed without a database CHECK', () => withTmpDir(dir => {
  ...
  assert.equal(inquiries.move(first.id, 'replied').status, 'replied', 'a mistaken close can be corrected')
  assert.equal(inquiries.move(first.id, 'new').status, 'new', 'a mistaken replied click can be corrected')
  assert.equal(inquiries.move(second.id, 'closed').status, 'closed', 'a message that needs no reply may close directly')
  assert.deepEqual(inquiries.counts(), { new: 1, replied: 0, closed: 1 })
  inquiries.move(first.id, 'closed')
