...
function CustomerRequest({ navigate }) {
  ...
  const continueFromVehicle = () => {
    ...
    if (!formData.vehicleInfo.trim()) {
      setStepError('Tell me what vehicle the tires are going on')
      return
    }
    setStepError('')
    setOrderStep(3)
  ...
  return (
    <div className="min-h-screen customer-shell">
      <nav className="site-nav"><button className="brand-mark" onClick={() => navigate('/')} aria-label="KMT home"><img src="/brand/kens-dark-600.webp" alt="Ken's Mobile Tire" width="600" height="513" /></button><div className="site-links"><button className="active" onClick={() => document.getElementById('order')?.scrollIntoView({ behavior: 'smooth' })}>Order Tires</button><button onClick={() => document.getElementById('services')?.scrollIntoView({ behavior: 'smooth' })}>Services</button><button onClick={() => navigate('/status')}>My Quote</button></div><a className="phone-link" href={TEXT_HREF}>{TEXT_LABEL}</a></nav>
      <section className="hero-section"><div className="hero-copy"><p className="eyebrow">I COME TO YOU</p><h1>Mobile Tire<br /><span>Service</span></h1><p className="hero-lede">Tires. Repairs. Roadside assistance.<br />Fast, reliable &amp; always on the move.</p><button className="hero-cta" onClick={() => document.getElementById('order')?.scrollIntoView({ behavior: 'smooth' })}>Order Tires <span>→</span></button></div><div className="hero-visual" aria-label="Ken's Mobile Tire brand"><img className="hero-logo" src="/brand/kens-dark-1200.webp" srcSet="/brand/kens-dark-600.webp 600w, /brand/kens-dark-1200.webp 1200w" sizes="(max-width: 760px) 72vw, 420px" width="1200" height="1027" alt="Ken's Mobile Tire logo" /><span className="hero-visual-label">REAL MOBILE SERVICE / BOSTON</span></div></section>
      <section className="service-strip" id="services"><div><strong>◉</strong><span><b>I COME TO YOU</b>Home, work or roadside</span></div><div><strong>↯</strong><span><b>FAST &amp; RELIABLE</b>Quick response you can count on</span></div><div><strong>✓</strong><span><b>QUALITY SERVICE</b>Professional care every time</span></div><button className="btn btn-neutral" onClick={() => navigate('/inquiry')}>More than tires? Tell me</button></section>
      <main className="order-section" id="order"><div className="section-heading"><p className="eyebrow">SHOP KMT</p><h2>Order tires online</h2><p>Find the right fit for your vehicle and I&apos;ll handle the rest.</p></div><div className="order-steps" aria-label="Order progress">{['Tire size', 'Your vehicle', 'Mobile service'].map((label, index) => <div className={orderStep === index + 1 ? 'order-step current' : orderStep > index + 1 ? 'order-step complete' : 'order-step'} key={label}><span>{index + 1}</span><b>{label}</b></div>)}</div>
