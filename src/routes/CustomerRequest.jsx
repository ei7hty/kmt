  ...
  return (
    <div className="min-h-screen customer-shell">
      ...
      {/* One tap to a text with the draft already written, in reach from
          ...
          LAST, AFTER THE FOOTER, AND IT HAS TO BE. The bar is
          `position: fixed`, so where it sits in the document costs nothing
          visually -- but `.forge/request-flow-check.mjs:156` asserts the
          social section's immediate siblings are `#order` before it and
          `.site-footer` after it. Putting this between the order form and
          the social section broke that check, which is how it was found. */}
