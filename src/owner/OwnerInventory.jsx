  ...
  return <div className="oi-shell">
    ...
    <main className="oi-content">
      ...
      <section className="oi-refresh" aria-label="Supplier refresh">
        ...
        <div className="oi-refresh-actions"><button className="oi-button oi-primary" onClick={refresh} disabled={!data || busy || jobRunning || (!size && !summary?.refreshableSizes.length)}>{size ? `Refresh ${size}` : `Refresh all ${summary?.refreshableSizes.length ?? ''} sizes with supplier data`}</button>{jobRunning && <button className="oi-button" onClick={cancel} disabled={busy}>Stop refresh</button>}</div>
      </section>
