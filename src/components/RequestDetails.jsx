...
export function VehicleDetails({ vehicle, onVehicleChange, value, onChange }) {
  return <section className="request-details" aria-labelledby="vehicle-heading">
    ...
    <p className="detail-help">Start with the year and make, then add your model. Unsure? Tell us what you know.</p>
...
export function ServiceDetails({ formData, onChange, errors }) {
  ...
  return <section className="request-details service-details" aria-labelledby="location-heading">
    <div className="detail-heading"><div><span className="detail-index">SERVICE LOCATION</span><h4 id="location-heading">We’ll meet you there.</h4></div></div>
    ...
    <div className="date-section"><div><label htmlFor="date">Preferred date</label><input id="date" type="date" name="date" value={formData.date} onChange={onChange} aria-invalid={!!errors.date} aria-describedby="date-help" /></div><div className="date-shortcuts">{[['Today', 0], ['Tomorrow', 1]].map(([label, offset]) => <button type="button" className="date-choice" key={label} aria-pressed={formData.date === dateOffset(offset)} onClick={() => update('date', dateOffset(offset))}>{label}</button>)}</div><p id="date-help" className="detail-help">{errors.date || 'Your preferred date is a request. Timing is confirmed separately.'}</p></div>
