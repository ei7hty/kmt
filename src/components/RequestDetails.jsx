...
export function ServiceDetails({ formData, onChange, errors, disposalFee }) {
  ...
  return <section className="request-details service-details" aria-labelledby="location-heading">
    ...
    {/* Opt-in only, and only offered once Ken has set a fee (pricing-settings.md,
        #289) -- disposalFee is null until he does, and null means "not offered",
        not "free." Default off: an opt-in that starts checked is not an opt-in. */}
    {disposalFee != null && <div className="disposal-option"><label><input type="checkbox" checked={!!formData.disposeOldTires} onChange={() => update('disposeOldTires', !formData.disposeOldTires)} /> Take away my old tires (+${disposalFee.toFixed(2)} per tire)</label></div>}
    <div className="date-section"><div><label htmlFor="date">Preferred date</label><input id="date" type="date" name="date" value={formData.date} onChange={onChange} min={dateOffset(MIN_LEAD_DAYS)} aria-invalid={!!errors.date} aria-describedby="date-help" /></div><div className="date-shortcuts">{[['In a week', MIN_LEAD_DAYS, 'in-a-week'], ['In two weeks', MIN_LEAD_DAYS * 2, 'in-two-weeks']].map(([label, offset, key]) => <button type="button" className="date-choice" data-testid={`date-shortcut-${key}`} key={label} aria-pressed={formData.date === dateOffset(offset)} onClick={() => update('date', dateOffset(offset))}>{label}</button>)}</div><p id="date-help" className="detail-help">{errors.date || 'I need a week’s notice, so the earliest I can come is a week out. Your preferred date is a request; timing is confirmed separately.'}</p><p className="detail-help date-sooner">If you need it today, text me.<a className="btn btn-neutral date-sooner-action" href={TEXT_HREF}>{TEXT_LABEL}</a></p></div>
