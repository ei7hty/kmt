...
| --- | --- | --- | --- |
| codex/request-list-stable-order | BUG FIXER (Codex 01a079eb) | `backend/quotes.mjs` only `listForCustomer` and `listForOwner` request ordering queries; `backend/quotes.test.mjs` a dedicated same-`created_at` ordering regression near existing list tests. Add a deterministic insertion-order tie-breaker after chronology for customer and owner views. DECLARED DISJOINT OVERLAP with PR #436: it owns catalog composition and an early catalog regression in these files; this claim does not touch those regions and will rebase/preserve them. No adapter, schema, API-shape, production/customer/Fly/provider/scraper action. | 2026-09-08 |
