...
| --- | --- | --- | --- |
| `codex/approved-image-serving` | KMT Image Adapter Remediation | `backend/image-*` private persistent storage, exact-five local ingestion, approval and serving modules/tests; `backend/inventory.mjs` approved image catalog projection; `backend/api.mjs`, `backend/server.mjs`, `backend/dev.mjs` image routing/composition only; `scripts/` local image operator tooling/fixtures; `scripts/scrape-tires.mjs` validation packet input/output only and `scripts/browser-fetch.mjs` product-tab resource policy only (PM-requested upstream bridge); `docs/image-mirroring.md` and image operator docs; `Dockerfile` decoder runtime provisioning/tests and `.dockerignore` / `.gitignore` private-image-store exclusion only (deployment-lane regions; independent-review privacy fix) | 2026-09-08 |
