...
| seo-analyst-role | SEO ANALYST (local_f596e88c) | new file .forge/roles/seo-analyst.md only, from the crawl-surface audit and PR #273. NOT .forge/AGENTS.md's lanes table -- index-head-lane-ruling already holds that for the same PM ruling; deferring the lanes row to avoid colliding with it. | 2026-09-06 |
| mail-templates-voice | PRODUCT MANAGER / OWNER AGENT (local_44d1e1f9) | backend/mail-templates.mjs and backend/mail.test.mjs — LANE EXCEPTION, declared: these are backend files and the voice is my lane. Three of four customer emails say "we", refer to Ken in the third person and carry no text number; t62-voice.md part F was written for these templates and never applied. Copy plus one guard test, no logic. On the user's direct instruction, ahead of their SMTP switch-on. | 2026-09-06 |
