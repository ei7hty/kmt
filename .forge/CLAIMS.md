...
| pricing-settings | JUNIOR BACK END DEV (local_0b9989ef) | src/markup.js (shippingPerTire), src/pricing.js (mobileServiceFee/disposalFee/tax settings, subtotal+tax quote fields), backend/inventory.mjs (getPricingSettings/savePricingSettings, saveMarkup shippingPerTire), backend/api.mjs (GET/PUT /api/owner/pricing, /api/catalog pricing field, PUBLIC_API paths unaffected), backend/quotes.mjs (disposeOldTires request field, CUSTOMER_QUOTE_FIELDS gains subtotal/tax), src/owner/OwnerInventory.jsx (pricing settings panel, markup panel gains shipping field), src/routes/CustomerRequest.jsx (disposal opt-in at service step) -- per .forge/pricing-settings.md (#289). NOT src/routes/QuoteRequests.jsx or src/routes/Status.jsx: tax ships disabled and disposal is a generic lineItems entry, so neither screen needs a change for this PR -- deliberately staying out of reject-confirm-and-decline-voice's region. | 2026-09-06 |
| `wait-loop-and-jobs-api` | DEV OPS/INFRASTRUCTURE | `.github/workflows/fly-deploy.yml`, `docs/operations.md` | 2026-09-06 |
