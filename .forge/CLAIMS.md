...
| --- | --- | --- | --- |
| `codex/approved-image-serving` | KMT Image Adapter Remediation | `backend/image-*` private persistent storage, exact-five local ingestion, approval and serving modules/tests; `backend/inventory.mjs` approved image catalog projection; `backend/api.mjs`, `backend/server.mjs`, `backend/dev.mjs` image routing/composition only; `scripts/` local image operator tooling/fixtures; `scripts/scrape-tires.mjs` validation packet input/output only and `scripts/browser-fetch.mjs` product-tab resource policy only (PM-requested upstream bridge); `docs/image-mirroring.md` and image operator docs; `Dockerfile` decoder runtime provisioning only (deployment-lane region) | 2026-09-08 |
